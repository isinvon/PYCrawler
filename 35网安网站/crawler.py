...
def get_res():
    ...
    for i in range(0,length):    
        ...
        # 名字
        tag_name = tag_main.select('a[class="xe-user-name overflowClip_1"]')[0].text.strip()
        # 链接
        tag_link = tag_main.select('div')[0].attrs.get('data-original-title').strip()
        # 图标
        if tag_main.select('img')[0].attrs.get('src').strip().replace(' ', '').__contains__('http'):
            tag_image = tag_main.select('img')[0].attrs.get('src').strip().replace(' ', '')
        else:
            tag_image = 'https://dh.aabyss.cn'+tag_main.select('img')[0].attrs.get('src').strip().replace(' ', '')
        # 描述
    ...
    # 将标签列表转换为 JSON 格式
    json_data = json.dumps(tag_list, ensure_ascii=False, indent=2)
